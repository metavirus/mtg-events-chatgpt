...
function eventCard(event, compact = false, options = {}) {
  ...
  return `<article class="event-card ${catalog ? 'catalog-event-card' : ''} ${dense ? 'dense-event-card' : ''} ${isCompetitive(event) ? 'competitive' : ''} ${(isHidden || isDisliked) ? 'deprioritized' : ''} ${isPrereleaseOrSealed(event) ? 'limited-highlight' : ''} ${emphasize ? `fit-${fit.tone}` : ''}" data-event-id="${escapeHtml(event.id)}" data-date="${dateKey(event.occurrenceDate)}" role="button" tabindex="0" aria-label="Open ${escapeHtml(event.title)} details">
    <div class="event-time"><strong>${escapeHtml(eventTimeLabel(event))}</strong><span>${event.recurrence?.frequency === 'weekly' ? 'Weekly' : 'One-off'}</span>${showDate && dateNote ? `<small>${dateNote}</small>` : ''}</div>
...
function highlightEvent(event) {
  const place = store(event.storeId);
  return `<button class="highlight-card" data-event-id="${event.id}" data-date="${dateKey(event.occurrenceDate)}"><span class="highlight-date"><strong>${event.occurrenceDate.getDate()}</strong>${event.occurrenceDate.toLocaleDateString(undefined, { month: 'short' })}</span><span><em>${isSpecial(event) ? 'Special event' : evidenceLabel(event).label}</em><strong>${escapeHtml(event.title)}</strong><small>${escapeHtml(place.name)} · ${escapeHtml(eventTimeLabel(event))}</small></span><span>→</span></button>`;
}
...
function seriesRow(event) {
  return `<button class="series-row" data-event-id="${event.id}" data-date="${event.date || event.startDate}"><span class="format-mark ${formatClass(event)}">${formatShort(event)}</span><span><strong>${escapeHtml(event.title)}</strong><small>${event.recurrence?.frequency === 'weekly' ? `${dayName(event.recurrence.dayOfWeek)} · ${escapeHtml(eventTimeLabel(event))} · recurring` : event.date || 'Dated event'}</small></span><span class="status-chip ${event.confidence === 'high' ? 'mint' : 'amber'}">${event.confidence}</span></button>`;
}
...
function renderCommunities() {
  ...
  const visibleHubs = (state.favoritesOnly ? formalHubs.filter(isCommunityHubFollowed) : formalHubs)
    .sort((a, b) => communityHubRank(b) - communityHubRank(a) || compareText(a.name, b.name));
...
function communityDigestRank(signal) {
  ...
  const relatedPlace = signal.relatedEntityType === 'venue' ? store(signal.relatedEntityId) : null;
  const affinity = personalAffinityScore({
    community: signal.relatedEntityType === 'community' ? community(signal.relatedEntityId) : sourceCommunity,
    place: relatedPlace || (linkedEvent ? store(linkedEvent.storeId) : null),
    signal
  });
  const freshness = datedUrgencyScore(observed, { halfLifeDays: 10, floor: -220, scale: 220 });
  const eventUrgency = linkedEvent ? occurrenceUrgencyScore(linkedEvent.occurrenceDate || parseDate(linkedEvent.date || linkedEvent.startDate)) : 0;
  const pastPenalty = linkedEvent && (linkedEvent.occurrenceDate || parseDate(linkedEvent.date || linkedEvent.startDate)) && (linkedEvent.occurrenceDate || parseDate(linkedEvent.date || linkedEvent.startDate)) < startOfDay(new Date())
    ? 160
    : 0;
  return (personal ? 420 : 0) + (plan ? 180 : 0) + affinity + freshness + eventUrgency + signalRank(signal) - pastPenalty;
}
...
function isActionableCommunityDigestSignal(signal) {
  ...
  if (observedAgeDays > 14 && !socialMemory && !linkedEvent) return false;
  return true;
}
...
function communityPlanMeta(event) {
  ...
  return `<div class="community-plan-meta"><span>${escapeHtml(occurrence.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }))}</span><strong>${escapeHtml(eventTimeLabel(event))}</strong><button data-place-id="${escapeHtml(place.id)}">${escapeHtml(place.name)}</button></div>`;
}
...
function recentPersonalActivityScore(matchers = []) {
  const history = state.personal.activity || [];
  if (!history.length || !matchers.length) return 0;
  return history.slice(0, 30).reduce((score, item, index) => {
    const label = String(item.label || '').toLowerCase();
    if (!matchers.some((matcher) => matcher && label.includes(matcher))) return score;
    return score + Math.max(4, 22 - index);
  }, 0);
}

function notePriorityScore(...keys) {
  return keys.reduce((score, key) => {
    const note = String(state.personal.notes?.[key] || '').trim();
    if (!note) return score;
    return score + Math.min(42, 18 + Math.ceil(note.length / 40) * 6);
  }, 0);
}

function ratingPriorityScore(...keys) {
  return keys.reduce((score, key) => {
    const rating = Number(state.personal.ratings?.[key] || 0);
    if (!rating) return score;
    if (rating <= 1) return score - 45;
    return score + (rating * 12);
  }, 0);
}
...
function datedUrgencyScore(dateLike, { halfLifeDays = 14, floor = -180, scale = 180 } = {}) {
  ...
  return Math.max(floor, Math.round(scale - ((ageDays / halfLifeDays) * scale)));
}

function occurrenceUrgencyScore(dateLike) {
  const date = dateLike instanceof Date ? dateLike : parseDate(dateLike);
  if (!date || Number.isNaN(date.getTime())) return 0;
  const today = startOfDay(new Date());
  const diffDays = Math.round((startOfDay(date).getTime() - today.getTime()) / (24 * 60 * 60 * 1000));
  if (diffDays < -1) return -120;
  if (diffDays <= 1) return 170;
  if (diffDays <= 7) return 120;
  if (diffDays <= 21) return 70;
  if (diffDays <= 56) return 28;
  return 0;
}

function personalAffinityScore({ community: targetCommunity = null, place = null, signal = null } = {}) {
  const communityKey = targetCommunity?.id ? `community:${targetCommunity.id}` : '';
  const placeKey = place?.id ? `place:${place.id}` : '';
  const eventKey = signal?.relatedEntityType === 'event' && signal.relatedEntityId ? `event:${signal.relatedEntityId}` : '';
  const keys = [communityKey, placeKey, eventKey].filter(Boolean);
  const favoriteScore = keys.reduce((score, key) => score + (state.personal.favorites?.[key] ? 54 : 0), 0);
  const noteScore = notePriorityScore(...keys);
  const ratingScore = ratingPriorityScore(...keys);
...
function communityHubRank(hub) {
  ...
  const favoriteBoost = state.personal.favorites?.[`community:${hub.community.id}`] ? 70 : 0;
  const noteBoost = notePriorityScore(`community:${hub.community.id}`, ...profile.locations.map((location) => location.place ? `place:${location.place.id}` : ''));
  const ratingBoost = ratingPriorityScore(`community:${hub.community.id}`, ...profile.locations.map((location) => location.place ? `place:${location.place.id}` : ''));
  const connectionBoost = profile.connections.length ? 90 : 0;
...
function openChangeEvents(changeId) {
  ...
  const eventRows = matchedEvents.length
    ? matchedEvents.map((event) => {
        const occurrence = event.occurrenceDate || parseDate(event.date || event.startDate);
        return `<button class="occurrence-row" data-event-id="${escapeHtml(event.id)}" ${occurrence ? `data-date="${dateKey(occurrence)}"` : ''}><time><strong>${occurrence ? occurrence.getDate() : '—'}</strong>${occurrence ? occurrence.toLocaleDateString(undefined, { month: 'short' }) : ''}</time><span><strong>${escapeHtml(event.title)}</strong><small>${occurrence ? occurrence.toLocaleDateString(undefined, { weekday: 'long' }) : 'Dated event'} · ${escapeHtml(eventTimeLabel(event))}</small></span><span class="status-chip ${evidenceLabel(event).tone}">${evidenceLabel(event).label}</span></button>`;
      }).join('')
...
function openEvent(id, occurrenceDate) {
  ...
  openDrawer(`<div class="drawer-kicker"><span class="format-mark ${formatClass(event)}">${formatShort(event)}</span>${organizer ? '<span class="status-chip sky">Community meetup</span>' : ''}<span class="status-chip ${fit.tone}">${fit.label}</span><span class="status-chip ${evidence.tone}">${evidence.label}</span>${hidden ? '<span class="status-chip coral">Hidden by you</span>' : ''}${disliked ? '<span class="status-chip coral">Not for you</span>' : ''}<span class="drawer-preference-actions"><button class="heart-button ${favorite ? 'active' : ''}" data-favorite="${personalKey}" aria-label="${favorite ? 'Unfollow event series' : 'Follow event series'}" title="${favorite ? 'Following series' : 'Follow series'}">${heartIcon()}</button><button class="visibility-button ${hidden ? 'active' : ''}" data-action="toggle-event-hidden" data-event-id="${event.id}" aria-label="${hidden ? 'Show event normally' : 'Hide event for now'}" title="${hidden ? 'Show normally' : 'Hide for now'}">${eyeClosedIcon()}</button><button class="thumb-button ${disliked ? 'active' : ''}" data-action="toggle-event-dislike" data-event-id="${event.id}" aria-label="${disliked ? 'Remove event dislike' : 'Dislike event series'}" title="${disliked ? 'Remove dislike' : 'Not for me'}">${thumbDownIcon()}</button></span></div><h1 id="drawerTitle">${escapeHtml(event.title)}</h1>${organizer ? `<button class="drawer-place-link" data-community-id="${escapeHtml(organizer.id)}">Organized by ${escapeHtml(organizer.name)} →</button><span class="drawer-attribution-separator"> · </span>` : ''}${hostAttribution}
    <div class="event-hero-meta"><div><span>Date</span><strong>${occurrence.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</strong></div><div><span>Time</span><strong>${escapeHtml(eventTimeLabel(event))}</strong></div><div><span>Entry</span><strong>${event.entryFee == null ? 'Unknown' : Number(event.entryFee) === 0 ? 'Free' : `$${event.entryFee}`}</strong></div><div><span>Power</span><strong>${event.bracket && event.bracket !== 'unspecified' ? `Bracket ${event.bracket}` : 'Not stated'}</strong></div></div>
