...
begin
  if p_source_type not in (
    ...
    raise exception 'unsupported official source type: %', p_source_type;
  end if;
  ...
  v_fingerprint := md5(concat_ws('|',
    ...
    p_attention_summary, p_suggested_action
  ));
  ...
  where r.idempotency_key = 'official-observation:' || p_run_key;
  ...
  if not found then
    insert into public.event_ingest_runs (
      ...
      'official-observation:' || p_run_key, 'official', 1, v_fingerprint,
      'live', 'staged'
  ...
  select exists (
    ...
    where o.source_family = 'official'
  ...
  insert into public.event_observations (
    ...
    format, event_type, occurrence_date, start_time, end_time, source_timezone,
    series_hint_key, source_status, fee_amount, fee_currency, bracket,
    proxy_policy, source_description, field_presence, source_native_payload,
    promotion_eligibility, reconcile_state, attention_category,
    attention_priority, attention_summary, suggested_action
  )
  select
    v_run.id, v_run.id, 'official', p_source_id, p_source_type, p_source_id,
    p_upstream_event_id, btrim(p_source_url), timezone('utc', now()),
    timezone('utc', now()), timezone('utc', now()), v_fingerprint,
    ...
    jsonb_build_object('sourceLabel', p_source_label), 'eligible', 'pending',
...
comment on function public.stage_official_event_observation(
  ...
  'Service-only typed adapter for one official store-controlled event observation. Canonical writes and presentation remain the shared promoter responsibility.';
