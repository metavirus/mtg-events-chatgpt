...
def split_generated_sql_statements(sql: str) -> list[str]:
    """Split SQL generated by this script into connector-ready statements.

    The generated apply SQL is intentionally one statement per line. This is not
    a general SQL parser; it is a guardrail for this workflow's own output.
    """
    statements: list[str] = []
    for line in sql.splitlines():
        ...
        if not stripped or stripped.startswith("--"):
            continue
        if stripped in {"begin;", "set constraints all immediate;", "commit;"}:
            continue
        if stripped.startswith("select * from (") or stripped.startswith("select series_id, occurrence_date, start_time, count(*)"):
            break
        statements.append(line)
    return statements


def connector_package_prefix(proposal: dict[str, Any], proposal_path: Path) -> str:
    raw = str(proposal.get("proposal_id") or proposal_path.stem)
    return re.sub(r"[^a-zA-Z0-9_.-]+", "-", raw).strip("-") or proposal_path.stem


def write_connector_package(
    proposal_path: Path,
    ...
    if chunk_size < 1:
        raise WorkflowError("--connector-chunk-size must be at least 1")
    output_dir.mkdir(parents=True, exist_ok=True)
    prefix = connector_package_prefix(proposal, proposal_path)
    statements = split_generated_sql_statements(sql)
    chunk_paths: list[Path] = []
    for start in range(0, len(statements), chunk_size):
        chunk_path = output_dir / f"{prefix}.apply-{len(chunk_paths) + 1:02d}.sql"
        chunk = statements[start:start + chunk_size]
        chunk_path.write_text("\n".join(chunk) + "\n", encoding="utf-8", newline="\n")
        chunk_paths.append(chunk_path)
    readback_path = output_dir / f"{prefix}.readback.sql"
    readback_path.write_text(verification_sql, encoding="utf-8", newline="\n")
    manifest = {
        "generatedBy": "scripts/supabase_research_workflow.py apply-approved --connector-package-dir",
        "generatedAt": datetime.now(timezone.utc).isoformat(),
        "proposal": str(proposal_path),
        "proposalId": proposal.get("proposal_id"),
        "risk": classification["risk"],
        "tablesTouched": classification["touched_tables"],
        "operationCounts": classification["operation_counts"],
        "applyChunks": [str(path) for path in chunk_paths],
        "readbackSql": str(readback_path),
        "postWriteLandingRule": (
            "Once a live write begins, do not explore tooling or workflow "
            "alternatives; apply only the prepared chunks, run readbacks, "
            "checkpoint, and report."
        ),
        "note": (
            "These files are connector-friendly execution artifacts generated "
            "from a validated proposal. Run apply chunks in numeric order "
            "through the approved Supabase SQL connector, then run the readback "
            "SQL. They are not canonical app JSON."
        ),
    }
    write_json(output_dir / f"{prefix}.connector-manifest.json", manifest)
    print(f"PASS connector apply package written: {output_dir}")
    print(f"Apply chunks: {len(chunk_paths)}")
    print(f"Readback SQL: {readback_path}")
...
def command_apply_approved(args: argparse.Namespace) -> int:
    ...
    connector_package_requested = bool(args.connector_package_dir)
    ...
    if connector_package_requested:
        write_connector_package(
            proposal_path=proposal_path,
            proposal=proposal,
            sql=sql,
            verification_sql=verification_sql,
            output_dir=Path(args.connector_package_dir),
            chunk_size=args.connector_chunk_size,
            classification=classification,
        )
        print("Post-write landing rule: once the first live chunk is executed, stop exploring tooling paths; verify, checkpoint, and report.")

    if not execution_requested:
        print("DRY RUN: no live write performed. Rerun with --execute and a database URL backend, or use the connector package with the approved Supabase SQL connector.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    apply.add_argument(
        "--connector-package-dir",
        help=(
            "Write connector-friendly numbered apply chunks, readback SQL, "
            "and a manifest for Supabase connector execution. Does not execute."
        ),
    )
    apply.add_argument(
        "--connector-chunk-size",
        type=int,
        default=25,
        help="Statements per connector apply chunk. Default: 25.",
    )
